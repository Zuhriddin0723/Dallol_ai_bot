...
def get_all_products():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name, min_price, max_price, sizes, colors, photo_id FROM products WHERE stock > 0")
...
def get_system_prompt():
    ...
    products_text = "\n".join([f"- {p[0]}: {p[1]} - {p[2]} so'm (O'lcham: {p[3]}, Rang: {p[4]})" for p in products]) if products else "Hozircha mahsulot yo'q."
...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    if user_id in pending_questions:
        ...
        if p_data:
            ...
            await process_ai_message(message, q['question'], state)
    else:
...
async def group_handler(message: types.Message):
    ...
    if is_reply_to_photo or any(word in raw_text for word in trigger_words):
        user_id = message.from_user.id
        # Agar rasmga reply bo'lsa, o'sha rasmning file_id si orqali mahsulotni topamiz
        photo_id = message.reply_to_message.photo[-1].file_id if is_reply_to_photo else None
        
        conn = sqlite3.connect("products.db")
        cursor = conn.cursor()
        
        # Avval local photos dan qidiramiz (file_id o'rniga file_name saqlangan bo'lishi mumkin)
        cursor.execute("SELECT name FROM products")
        all_p = cursor.fetchall()
        p_name = None
        for p in all_p:
            if os.path.exists(f"photos/{p[0]}.jpg"):
                # Bu mantiq biroz sodda, asosiysi rasmga reply qilinganda mahsulotni tanish
                p_name = p[0]
                break
        
        if not p_name and photo_id:
            cursor.execute("SELECT name FROM products WHERE photo_id = ?", (photo_id,))
            row = cursor.fetchone()
            p_name = row[0] if row else None
        ...
        await message.reply(f"@{message.from_user.username}, lichkangizda javob berdim!", reply_markup=markup)

async def process_ai_message(message, text, state):
    ...
    try:
        ...
        if photo_match:
            ...
            if p_photo_file and os.path.exists(f"photos/{p_photo_file}"):
                await bot.send_photo(message.chat.id, FSInputFile(f"photos/{p_photo_file}"))
        ...
        if "[DEAL_REACHED]" in res_text:
            ...
            if prices: await state.update_data(last_price=int(prices[-1].replace(" ", "")))
            await message.answer(res_text.replace("[DEAL_REACHED]", "").strip())
            await message.answer("Kelishdik! Ismingizni yozing:")
        ...
        await message.answer("Kechirasiz, birozdan keyin urinib ko'ring.")
...
async def private_handler(message: types.Message, state: FSMContext):
    if message.text: await process_ai_message(message, message.text, state)
